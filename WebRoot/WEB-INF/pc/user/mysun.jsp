			...
			success : function(re){
				if(re!=null){
					if(re.status===200 && re.list.length > 0){
						...
						$('.mysunul').html(strVar);
						$('.page_p').remove();
						$('.mysunul').append('<p class="page_p"></p>');
						if(re.countPage > 1){
							$('.page_p').createPage({
								pageCount : re.countPage,
								current : re.start,
								input_name : 'mysun-input',
								backFn : function(p){
									jume(p,re.size);
								}
				            });
						...
						$('.mysunul').html("<h2 style=\"text-align: center;font-size: 20px;\">您还未发表任何晒家，请前去发布吧</h2>");
					}
				}
			},error : function(re){
				alert(re.status);
