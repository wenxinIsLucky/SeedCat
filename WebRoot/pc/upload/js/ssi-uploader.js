...
//----------------------------UPLOADFILES------------------------------------
            ...
            if (thisS.options.preview) {
                ...
                if (fileType[0] == 'image') {
                    ...
                    fileReader.onload = function () {
                        imgContent += getTemplate('<img class="ssi-imgToUpload" name="files" src=""/><i class="fa-spin fa fa-spinner fa-pulse"></i>'); // set the files element without the img
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                msg += '<input class="imgs" type="hidden" name="imgs" value="'+data+'"/>'
                if (data == "FAIL") {
                	title = "上传失败！";
                }
    ...
    var locale = {
        en: {
            ...
            clear: '取消',
            drag: '图片预览区',
            sizeError: '超过尺寸限制',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '不支持文件类型',//$1=file extension ie(exe files are not supported)
